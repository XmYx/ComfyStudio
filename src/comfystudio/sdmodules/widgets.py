...
class ReorderableListWidget(QWidget):
    ...
    A Reorderable ListWidget that behaves similarly to a DaVinci Resolve style hover-scrub:
     ...
     - In/Out points can be set by pressing 'I' or 'O' while hovering over an item
     - Zoom slider at top to control icon size
     - Items can be drag-reordered internally (using built-in QListWidget 'InternalMove')
     - Supports toggling between a "Grid" (IconMode) and "List" (ListMode) view
       and ensures the preview (icon) maintains aspect ratio and shot details are shown.
    """
    def __init__(self, parent=None):
        ...
        # Hover scrub data
        # We'll store item-specific data keyed by id(item)
        self.hover_fraction_map = {}  # item_id -> fraction [0..1]
        self.in_point_map = {}        # item_id -> in fraction
        self.out_point_map = {}       # item_id -> out fraction
        ...
        self.listWidget.setIconSize(QSize(120, 90))
        self.listWidget.setSpacing(10)
        # Enable built-in reordering
        self.listWidget.setDragEnabled(True)
        self.listWidget.setAcceptDrops(True)
        self.listWidget.setDropIndicatorShown(True)
        self.listWidget.setDragDropMode(QListWidget.InternalMove)
        # Allow multiple selection if desired
        self.listWidget.setSelectionMode(QListWidget.ExtendedSelection)
    ...
    def updateTexts(self):
        ...
        current_view_mode = self.listWidget.viewMode()
        if current_view_mode == QListView.IconMode:
            self.view_switch_button.setText(
                self.localization.translate("button_switch_to_list_view", default="Switch to List View")
            )
        else:
            self.view_switch_button.setText(
                self.localization.translate("button_switch_to_grid_view", default="Switch to Grid View")
            )
    ...
    def onZoomChanged(self, value):
        ...
        if self.listWidget.viewMode() == QListView.IconMode:
            # Grid mode: set grid size to fit icon + spacing
            self.listWidget.setGridSize(
                QSize(icon_size.width() + spacing, icon_size.height() + spacing)
            )
        else:
            # List mode: set grid size to accommodate a single row (image + text)
            self.listWidget.setGridSize(
                QSize(self.listWidget.viewport().width(), icon_size.height() + spacing + 50)
            )

    def mouseMoveEvent(self, event):
        # Convert local position to listWidget coords
        pos_in_list = self.listWidget.mapFrom(self, event.pos())
        item = self.listWidget.itemAt(pos_in_list)
        if item:
            self.current_hover_item = item
            ...
            rect = self.listWidget.visualItemRect(item)
            if rect.width() > 0:
                # fraction = (local_x - itemRect.x()) / itemRect.width()
                fraction = (pos_in_list.x() - rect.x()) / float(rect.width())
                fraction = max(0.0, min(1.0, fraction))
                self.hover_fraction_map[item_id] = fraction
        else:
    ...
    def onListWidgetContextMenu(self, pos):
        """
        Pass the context menu event to the parent window, if it exists.
        """
        if hasattr(self.parent_window, "onListWidgetContextMenu"):
            self.parent_window.onListWidgetContextMenu(pos)
    ...
    def onSelectionChanged(self):
        """
        Pass the selection-changed event to the parent window, if it exists.
        """
        if hasattr(self.parent_window, "onSelectionChanged"):
            self.parent_window.onSelectionChanged()
    ...
    def updateItems(self, shots: List[Shot]):
        """
        Populate the list with new items based on the provided shot list.
        """
        self.clearItems()
        for i, shot in enumerate(shots):
            if hasattr(self.parent_window, "getShotIcon"):
...
class HoverScrubList(QListWidget):
    ...
    Subclass of QListWidget to handle painting a 'timeline handle' or
    'hover fraction' marker, in the style of a DaVinci Resolve hover-scrub.
    Also draws a prominent border around selected items and
    displays shot details on the right in ListMode.
    """
    def __init__(self, reorderable_parent):
        super().__init__()
    ...
    def paintEvent(self, event):
        ...
        for i in range(self.count()):
            ...
            # Draw background if selected
            if is_selected:
                painter.fillRect(item_rect, QColor(200, 200, 255, 100))  # light blue overlay

            # Retrieve the associated Shot object
            shot_idx = item.data(Qt.ItemDataRole.UserRole)
            shot = self.reorderable_parent.parent_window.shots[shot_idx]
            if shot is None:
                # If for some reason there's no shot, skip
                continue

            # Draw the item icon, maintaining aspect ratio
            pixmap = item.icon().pixmap(self.iconSize())
            if not pixmap.isNull():
                if self.viewMode() == QListWidget.IconMode:
                    # Scale icon to fit item rect
                    scaled_pixmap = pixmap.scaled(
                        item_rect.size(),
                    ...
                    # Center the icon
                    x = item_rect.x() + (item_rect.width() - scaled_pixmap.width()) // 2
                    y = item_rect.y() + (item_rect.height() - scaled_pixmap.height()) // 2
                    painter.drawPixmap(x, y, scaled_pixmap)

                    # Draw shot name near bottom
                    name_rect = QRect(
                        item_rect.x() + 2,
                        item_rect.bottom() - 20,  # allocate ~20 px for text
                        item_rect.width() - 4,
                        18
                    )
                    ...
                    painter.drawText(
                        name_rect,
                        Qt.AlignmentFlag.AlignHCenter | Qt.TextFlag.TextSingleLine,
                        shot.name or "Untitled"
                    )

                else:
                    # ListMode: image on the left, details on the right
                    image_width = int(item_rect.height() * 16 / 9)  # 16:9 assumption
                    image_rect = QRect(item_rect.x(), item_rect.y(), image_width, item_rect.height())

                    # Scale the pixmap to the allocated image rect
                    scaled_pixmap = pixmap.scaled(
                        image_rect.size(),
                        Qt.AspectRatioMode.KeepAspectRatio,
                        Qt.TransformationMode.SmoothTransformation
                    ...
                    # Draw shot details on the right
                    details_x = image_rect.right() + 10
                    details_y = item_rect.y() + 10
                    details_width = item_rect.width() - image_rect.width() - 20
                    details_height = item_rect.height() - 20
                    ...
                    painter.setPen(Qt.GlobalColor.black)
                    painter.drawText(
                        QRect(details_x, details_y, details_width, details_height),
                        Qt.TextFlag.TextWordWrap,
                        details_text
                    )

            # Draw a red hover-scrub handle (fraction)
            item_id = id(item)
            fraction = self.reorderable_parent.hover_fraction_map.get(item_id, None)
            if fraction is not None:
                if self.viewMode() == QListWidget.IconMode:
                    handle_x = int(item_rect.x() + item_rect.width() * fraction)
                    painter.setPen(QPen(QColor(255, 0, 0), 3))
                    painter.drawLine(handle_x, item_rect.y(), handle_x, item_rect.bottom())
                else:
                    # In ListMode, handle is relative to the image section
                    image_width = int(item_rect.height() * 16 / 9)
                    handle_x = int(item_rect.x() + image_width * fraction)
                    painter.setPen(QPen(QColor(255, 0, 0), 2))
                    painter.drawLine(handle_x, item_rect.y(), handle_x, item_rect.bottom())
            ...
            if in_frac is not None:
                if self.viewMode() == QListWidget.IconMode:
                    ix = int(item_rect.x() + item_rect.width() * in_frac)
                else:
                    image_width = int(item_rect.height() * 16 / 9)
                    ix = int(item_rect.x() + image_width * in_frac)
                painter.setBrush(QColor(0, 255, 0))  # green
                painter.setPen(Qt.PenStyle.NoPen)
                painter.drawEllipse(ix - 4, item_rect.y() + item_rect.height() // 2 - 4, 8, 8)

            out_frac = self.reorderable_parent.out_point_map.get(item_id, None)
            if out_frac is not None:
                if self.viewMode() == QListWidget.IconMode:
                    ox = int(item_rect.x() + item_rect.width() * out_frac)
                else:
                    image_width = int(item_rect.height() * 16 / 9)
                    ox = int(item_rect.x() + image_width * out_frac)
                painter.setBrush(QColor(255, 165, 0))  # orange
                painter.setPen(Qt.PenStyle.NoPen)
                painter.drawEllipse(ox - 4, item_rect.y() + item_rect.height() // 2 - 4, 8, 8)

            # Draw a bright border if selected
            if is_selected:
                pen = QPen(QColor(0, 120, 215), 3)  # bright blue border
                painter.setPen(pen)
                painter.drawRect(item_rect.adjusted(1, 1, -2, -2))
